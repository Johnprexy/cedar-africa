...
onMounted(async () => {
  ...
  L.marker([6.4352, 3.4737], { icon }).addTo(map)

  // Drone survey boundary polygon
  const bounds = [
    [6.4365, 3.4720], [6.4365, 3.4755],
    [6.4338, 3.4755], [6.4338, 3.4720],
  ]
  L.polygon(bounds, { color: '#C9A84C', weight: 1.5, fillColor: '#C9A84C', fillOpacity: 0.07, dashArray: '6 4' }).addTo(map)
  ...
  L.marker([6.4362, 3.4748], { icon: icon2 }).addTo(map)
})
</script>
...
.tech-badge {
  ...
  font-family: var(--font-ui); font-size: 0.65rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase;
  color: rgba(247,242,232,0.6);
  background: rgba(247,242,232,0.04);
  border: 1px solid rgba(247,242,232,0.1);
  padding: 6px 12px;
}
.badge-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--tech-blue); box-shadow: 0 0 6px var(--tech-blue); animation: blink 2s ease-in-out infinite; }
...
.map-topbar {
  ...
  font-family: var(--font-ui); font-size: 0.7rem; color: rgba(247,242,232,0.5);
...
#hero-map { width: 100%; height: 320px; }
...
.pin-pulse {
  width: 14px; height: 14px; border-radius: 50%; background: var(--gold); flex-shrink: 0; margin-top: 2px;
...
@media (max-width: 1060px) {
  ...
  .hero-map-preview { max-width: 600px; }
  .scroll-cue { display: none; }
}
@media (max-width: 600px) {
  .hero { padding: 100px 20px 60px; }
  .hero-badges { left: 20px; top: 95px; }
